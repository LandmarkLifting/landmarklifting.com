...
/**
 * Cloudflare evaluates `_redirects` in order, and treats every rule from the
 * first splat onwards as a "dynamic" rule — of which only 100 are allowed.
 * With splats in the middle, later rules were silently dropped. So: every exact
 * rule first, splats last. Exact rules have a far higher ceiling.
 */
const exact = [];
const splat = [];
...
const lines = [
  ...
  '#',
  '# Exact rules come first and wildcard rules last: Cloudflare counts every',
  '# rule after the first splat as "dynamic" and only honours 100 of those.',
  '',
];
